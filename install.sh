...
usage() {
    cat <<HELP
Usage: bash install.sh [--version v2026.7.30|latest] [--profile recommended|core] [--extras name[,name]]

Environment equivalents:
  AGENTOS_VERSION=v2026.7.30
...
if [[ "${release_selector}" != "latest" && "${release_selector}" != "stable" ]] && ! is_release_version "${release_selector}"; then
    ...
    echo "install.sh: the release installer only supports latest, stable, or release versions like v2026.7.30." >&2
